...
import { playersSelector } from '../../features/playersSlice'
import CheckersHolder from '../CheckersHolder/CheckersHolder'
import { outsideCheckersSelector } from '../../features/boardSlice'
        ...
        <div className={styles.outsideLine} style={{ backgroundColor: bgc }}>

            {
                Object.keys(players).map(k => {
                    return (
                        <div><CheckersHolder position={players[k].color === COLORS.PLAYER_2? POSITION.BOTTOM: POSITION.TOP}  checkers={outsideCheckers[players[k].color]} checkersColor={players[k].color} /></div>
                    )
                })
            }
        </div>
