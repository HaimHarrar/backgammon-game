...
import { currentPlayerSelector, playersSelector } from '../../features/playersSlice'
import CheckersHolder from '../CheckersHolder/CheckersHolder'
import { middleCheckersSelector } from '../../features/boardSlice'
    ...
    return (
        <div className={styles.middleLine} style={{ backgroundColor: bgc }}>
            <div className={styles.currTurn}>
                <p>Turn: {currPlayer.username}</p>
            </div>
            <div className={styles.middleCheckersHolder}>
                {
                    Object.keys(players).map(k => {
                        return (
                            <div><CheckersHolder position={players[k].color === COLORS.PLAYER_2? POSITION.BOTTOM: POSITION.TOP} checkers={middleCheckers[players[k].color]} checkersColor={players[k].color} /></div>
                        )
                    })
                }
            </div>
